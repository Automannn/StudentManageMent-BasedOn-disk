	...
	public Userlogin(Display display) {
        ...
        btnNewButtonok.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				User user=new User(textuser.getText().trim(), textpass.getText().trim(), intActor);
				Shell oldshell=null;
				if (new UserSet().isValid(user)) {
					oldshell=shell;
					if (user.getJob()==User.STUDENT) {
						shell=new StdscoreMana(user).getShell();
					}
				if (user.getJob()==User.TEACHER) {
						shell=new TchscoreMana(user).getShell();
					}  
					if (user.getJob()==User.ADMIN) {
						shell=new AdminscoreMana(user).getShell();
					}
					oldshell.dispose();
					shell.open();
				}
